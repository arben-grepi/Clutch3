/**
 * Calculate last 50 shots percentage (from last 5 completed videos)
 */
export const calculateLast50ShotsPercentage = (files) => {
...
/**
 * Calculate last 100 shots percentage (from last 10 completed videos)
 */
...
/**
 * Calculate all-time stats from all completed videos
 */
export const calculateAllTimeStats = (files) => {
  if (!files || files.length === 0)
    return { percentage: 0, totalShots: 0, madeShots: 0 };
  ...
  // Calculate total shots: each video = 10 shots
  const totalShots = completedFiles.length * 10;
  
  // Calculate total made shots from all completed videos
